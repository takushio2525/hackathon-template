...
\verb|report/**/*.tex|（および \verb|.bib|, \verb|.sty|, \verb|.cls|）に変更のある push で自動的に \verb|latexmk| が走る。
\begin{itemize}
  \item \textbf{push の場合}: 生成された \verb|report/main.pdf| が自動で同じブランチにコミットされる（コミッタは \verb|github-actions[bot]|）。push 直後に手元で \verb|git pull| を実行して反映を取り込むこと。
  \item \textbf{Pull Request の場合}: PDF は Artifact にアップロードされる。Actions タブから \verb|report-pdf| としてダウンロード可能。
\end{itemize}
